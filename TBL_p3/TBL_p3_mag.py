...
theta3t = np.pi*7./6.  -thetacell 
theta4t = -np.pi/6.    -thetacell    
theta5t = np.pi/2.     -thetacell  
theta8t = np.pi*7./6.  -thetacell 
theta6t = -np.pi/6.    -thetacell    
...
for x in range(N):
    for y in range(N):
        ...
        vocc = v[:,:Nocc]
        Mx += -2.*np.trace(np.linalg.multi_dot([vocc.T.conjugate(),sx,vocc]))
        My += -2.*np.trace(np.linalg.multi_dot([vocc.T.conjugate(),sy,vocc]))
        Mz += -2.*np.trace(np.linalg.multi_dot([vocc.T.conjugate(),sz,vocc]))
